...
class LlamaTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        label_outputs = model(input_ids=input_ids,attention_mask=attention_mask,labels=labels)
        #  task rationale gen
        rationale_input_ids = inputs['rationale_input_ids']
        rationale_attention_mask = inputs['rationale_attention_mask']
        rationale = inputs['rationales']
        rationale_outputs = model(input_ids=rationale_input_ids,attention_mask=rationale_attention_mask,labels=rationale)

        loss_l = label_outputs["loss"] if isinstance(label_outputs, dict) else label_outputs[0]
        loss_r = rationale_outputs["loss"] if isinstance(rationale_outputs, dict) else rationale_outputs[0]

        loss = loss_l + self.args.gamma* loss_r
    ...
    def prediction_step(
        ...
        gen_kwargs["max_length"] = inputs['gen_input_ids'].shape[1] + gen_kwargs["max_length"]
        gen_kwargs["max_length"] = gen_kwargs["max_length"].item() if isinstance(gen_kwargs["max_length"],torch.Tensor) else gen_kwargs["max_length"]
        generated_tokens = self.model.generate(
            input_ids = inputs['gen_input_ids'],
            attention_mask = inputs['gen_attention_mask'],
            **gen_kwargs,
        )
        generated_tokens = generated_tokens[:,-self.model.config.max_length:]
